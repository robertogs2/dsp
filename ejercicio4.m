...
%Obtiene las componentes par e impar de una se̱al
%Entradas:
%x : vector de entrada con valores de la se̱al
...
function [x_par, x_impar, n_comp] = componentes(x, x_pos)
    ...
    k = length(n_comp);%tama̱o de la cantidad de indices para las funciones par e impar
...
%Grafica la se̱al de entrada, su componente par e impar y la suma de ambas
%Entradas:
%x : vector de entrada con valores de la se̱al
%x_pos : vector con valores enteros que indican la posicion de cada uno de los valores de la entrada x
...
function graficar = plotear(x, x_pos)
    ...
    x_comp_inicio=min(n_comp);%indice inicial es el indice mas peque̱o de todos los indices de las componentes
